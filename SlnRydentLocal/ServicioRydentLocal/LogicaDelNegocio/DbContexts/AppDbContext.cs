...
public class AppDbContext : DbContext
{
	...
	{
		// Constructor vacío para código legado que hace: new AppDbContext()
		// Usará OnConfiguring más abajo si no hay opciones configuradas.
	}

	public AppDbContext(DbContextOptions<AppDbContext> options) : base(options)
	{
		// Constructor que usa DI: Program.cs -> AddDbContext(...)
	}
	...
	public async Task<List<P_BUSCARPACIENTE>> P_BUSCARPACIENTE(int TIPO, string P_VALOR)
	{
		...
		var s = await this.P_BUSCARPACIENTE_Result
			.FromSqlRaw("select distinct * from P_BUSCARPACIENTE(@TIPO, @P_VALOR)", TIPOParameter, P_VALORParameter)
			.ToListAsync();
		return s;
	}
	...
	{
		var s = this.P_DIAN_FACTURASPENDIENTES_Results
			.FromSqlRaw("select * from P_DIAN_FACTURASPENDIENTES_D")
			.ToList();
		return s;
	}
	...
	public List<P_DIAN_FACTURASCREADAS_Results> P_DIAN_FACTURASCREADAS(string pFactura)
	{
		...
		var s = this.P_DIAN_FACTURASCREADAS_Results
			.FromSqlRaw("select * from P_DIAN_FACTURASCREADAS_D(@PFACTURA)", PFACTURAParameter)
			.ToList();
		return s;
	}
	...
	public List<SP_RIPS_JSON_APResult> SP_RIPS_JSON_AP(DateTime FECHAINI, DateTime FECHAFIN, string EPS, string FACTURA, int IDREPORTE, int IDDOCTOR, string EXTRANJERO)
	{
		...
		var s = this.SP_RIPS_JSON_APResult.FromSqlRaw(
			"select * from SP_RIPS_JSON_AP(@FECHAINI, @FECHAFIN, @EPS, @FACTURA, @IDREPORTE, @IDDOCTOR, @EXTRANJERO)",
			FECHAINIParameter, FECHAFINParameter, EPSParameter, FACTURAParameter, IDREPORTEParameter, IDDOCTORParameter, EXTRANJEROParameter
		).ToList();
		return s;
	}
	...
	public List<SP_RIPS_JSON_USResult> SP_RIPS_JSON_US(DateTime FECHAINI, DateTime FECHAFIN, string EPS, string FACTURA, int IDREPORTE, int IDDOCTOR, string EXTRANJERO)
	{
		...
		var IDDOCTORParameter = new FbParameter("IDDOCTOR", IDDOCTOR);
		var EXTRANJEROParameter = new FbParameter("EXTRANJERO", EXTRANJERO);
		var s = this.SP_RIPS_JSON_USResult.FromSqlRaw(
			"select * from SP_RIPS_JSON_US(@FECHAINI, @FECHAFIN, @EPS, @FACTURA, @IDREPORTE, @IDDOCTOR, @EXTRANJERO)",
			FECHAINIParameter, FECHAFINParameter, EPSParameter, FACTURAParameter, IDREPORTEParameter, IDDOCTORParameter, EXTRANJEROParameter
		).ToList();
		return s;
	}
	...
	public List<SP_RIPS_JSON_ACResult> SP_RIPS_JSON_AC(DateTime FECHAINI, DateTime FECHAFIN, string EPS, string FACTURA, int IDREPORTE, int IDDOCTOR, string EXTRANJERO)
	{
		...
		var IDDOCTORParameter = new FbParameter("IDDOCTOR", IDDOCTOR);
		var EXTRANJEROParameter = new FbParameter("EXTRANJERO", EXTRANJERO);
		var s = this.SP_RIPS_JSON_ACResult.FromSqlRaw(
			"select * from SP_RIPS_JSON_AC(@FECHAINI, @FECHAFIN, @EPS, @FACTURA, @IDREPORTE, @IDDOCTOR, @EXTRANJERO)",
			FECHAINIParameter, FECHAFINParameter, EPSParameter, FACTURAParameter, IDREPORTEParameter, IDDOCTORParameter, EXTRANJEROParameter
		).ToList();
		return s;
	}
	...
	{
		var s = await this.Generador_Result
			.FromSqlRaw("SELECT GEN_ID(GEN_CONSECUTIVO, 1) NUMERO FROM rdb$database")
			.FirstOrDefaultAsync();
	...
	{
		var s = await this.Generador_Result
			.FromSqlRaw($"SELECT GEN_ID({Nombre}, 1) NUMERO FROM rdb$database")
			.FirstOrDefaultAsync();
		return s.NUMERO.ToString();
	}
	...
	public async Task<List<P_CONSULTAR_ESTACUENTAPACIENTE>> P_CONSULTAR_ESTACUENTAPACIENTE(int IDANAMNESIS)
	{
		...
		var s = await this.P_CONSULTAR_ESTACUENTAPACIENTE_Result
			.FromSqlRaw("select * from P_CONSULTAR_ESTACUENTAPACIENTE(@IDANAMNESIS)", IDANAMNESISParameter)
			.ToListAsync();
		return s;
	}
	...
	public async Task<List<P_CONSULTAR_ESTACUENTA_TOTAL_F>> P_CONSULTAR_ESTACUENTA_TOTAL_F(int IDDOCTOR, DateTime IN_FECHA_INICIO, int IN_CUOTAS, DateTime FECHA_FIN)
	{
		...
		var s = await this.P_CONSULTAR_ESTACUENTA_TOTAL_F_Result
			.FromSqlRaw("select * from P_CONSULTAR_ESTACUENTA_TOTAL_F(@IDDOCTOR, @IN_FECHA_INICIO, @IN_CUOTAS, @FECHA_FIN)",
				IDDOCTORParameter, IN_FECHA_INICIOParameter, IN_CUOTASParameter, FECHA_FINParameter)
			.ToListAsync();
		return s;
	}
	...
	public async Task<List<P_CONSULTAR_ESTACUENTA_TOTAL>> P_CONSULTAR_ESTACUENTA_TOTAL(int IDDOCTOR, DateTime IN_FECHA_INICIO, int IN_CUOTAS)
	{
		...
		var IN_CUOTASParameter = new FbParameter("IN_CUOTAS", IN_CUOTAS);
		var s = await this.P_CONSULTAR_ESTACUENTA_TOTAL_Result
			.FromSqlRaw("select * from P_CONSULTAR_ESTACUENTA_TOTAL(@IDDOCTOR, @IN_FECHA_INICIO, @IN_CUOTAS)",
				IDDOCTORParameter, IN_FECHA_INICIOParameter, IN_CUOTASParameter)
			.ToListAsync();
		return s;
	}
	...
	public async Task<List<P_CONSULTAR_ESTACUENTA>> P_CONSULTAR_ESTACUENTA(int ID, int FASE, int IDDOCTOR)
	{
		...
		var IDDOCTORParameter = new FbParameter("IDDOCTOR", IDDOCTOR);
		var s = await this.P_CONSULTAR_ESTACUENTA_Result
			.FromSqlRaw("select * from P_CONSULTAR_ESTACUENTA(@ID, @FASE, @IDDOCTOR)",
				IDParameter, FASEParameter, IDDOCTORParameter)
			.ToListAsync();
		return s;
	}
	...
	public async Task<P_CONSULTAR_MORA_ID_TEXTO> P_CONSULTAR_MORA_ID_TEXTO(string ID_TEXTO)
	{
		...
		var s = await this.P_CONSULTAR_MORA_ID_TEXTO_Result
			.FromSqlRaw("select * from P_CONSULTAR_MORA_ID_TEXTO(@ID_TEXTO)", ID_TEXTOParameter)
			.FirstOrDefaultAsync();
		return s;
	}
	...
	public async Task<List<P_AGENDA1>> P_AGENDA1(string IN_SILLA, DateTime IN_FECHA, string IN_TIPO, string HORAINI, string HORAFIN, int INTERVALO, string PARARINI, string PARARFIN)
	{
		...
		try
		{
			var s = await this.P_AGENDA1_Result.FromSqlRaw(
				"select * from P_AGENDA1(@IN_SILLA, @IN_FECHA, @IN_TIPO, @HORAINI, @HORAFIN, @INTERVALO, @PARARINI, @PARARFIN)",
				IN_SILLAParameter, IN_FECHAParameter, IN_TIPOParameter, HORAINIParameter, HORAFINParameter, INTERVALOParameter, PARARINIParameter, PARARFINParameter
			).ToListAsync();
			return s;
		}
		catch (Exception)
	...
	private static string ConvertToISO8859_1(string input)
	{
		...
		var utfBytes = Encoding.UTF8.GetBytes(input);
		var isoBytes = Encoding.Convert(Encoding.UTF8, Encoding.GetEncoding("ISO-8859-1"), utfBytes);
	...
	{
		if (input == null) return null;
		var isoBytes = Encoding.GetEncoding("ISO-8859-1").GetBytes(input);
		var utfBytes = Encoding.Convert(Encoding.GetEncoding("ISO-8859-1"), Encoding.UTF8, isoBytes);
		return Encoding.UTF8.GetString(utfBytes);
	}

	protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
	{
		// Solo configurar aquí si NO vino por DI (AddDbContext)
